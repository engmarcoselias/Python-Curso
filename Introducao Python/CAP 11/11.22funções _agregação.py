    ...
    print("\nBrasil: {0:6} {1:18,} {2:10,} {3:10,.0f} {4:13,}".format(
        *conexão.execute("""
            select count(*), min(população), max(população),
                   avg(população), sum(população) from estados""").fetchone()))#na segunda consulta não utilizamos a clausa group by fazendo com que todos os registros façam parte do grupo